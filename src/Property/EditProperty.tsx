...
>((props, ref) => {
  useImperativeHandle(ref, () => ({
    Action() {
      Initialize();
      setNewProperty(props.codeTypes);
    },
  }));
...
const presetPropertyType =()=>{  
  const dataproptype= srcPropertyTypes?.find(x=>x.shortCode===props.codeTypes.propertyType);
  setPropertyType(dataproptype);    
}
const presetFurnish =()=>{
 
  const datafur= srcFurnished?.find(x=>x.shortCode===props.codeTypes.furnishedStatus);
  setFurnished(datafur);  
}
 
const presetDoorfacing =()=>{
  const datadoor = srcFacing?.find(x=>x.shortCode===props.codeTypes.facing);
  setFacing(datadoor);   
}
  ...
  */
  ...
  function createPost() {
    ...
    axios
      .put(appBaseURL + "/api/Property", newProperty)
      .then((response) => {
